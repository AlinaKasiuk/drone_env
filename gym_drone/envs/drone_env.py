...
class DroneEnv(gym.Env):
    ...
    def __init__(self):
      ...
      self.action_space = spaces.Box(low=-1.0, high=2.0, shape=(3, 4), dtype=np.float32) #düzelt
      self.observation_space = spaces.Box(low=-1.0, high=2.0, shape=(3, 4), dtype=np.float32) #düzelt
    ...
    def reset(self):
      ...
      self.droneid =p.loadURDF(os.path.join(urdfRootPath, "Drone.urdf"))
      ...
      state_robot = p.getBasePositionAndOrientation(droneId)  #dzüelt
    ...
    def step(self, action):
        ...
        m = p.calculateInverseDynamics(self.droneid, position, velocity, action)
